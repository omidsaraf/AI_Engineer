...
> Each phase lists **Inputs → Actions → Outputs → Validation Gate**. Follow in order. Items in *italics* reference sections/snippets elsewhere in this blueprint.

### Phase −1 — Scope, SLOs, Policies

**Inputs**: Business goals, data domains, risk posture.
**Actions**: Finalize SLOs (p95 ≤ 2.5s; GE pass ≥ 99%; hit‑rate ≥ 0.85; cost≤budget); approve RBAC model; data retention.
**Outputs**: Signed ADRs, SLO sheet, RBAC matrix.
**Validation**: Stakeholder sign‑off; tracked in repo `/docs/adr/`.

### Phase 0 — Prereqs & Naming

**Inputs**: Azure subscription, Terraform/Bicep access, GitHub org, SPNs.
**Actions**: Create RGs `rg-niloomid-{env}`, workspaces `niloomid-{env}`, set naming & tags (CostCenter/Owner).
**Outputs**: Subscription IDs, workspace URLs, SPN IDs.
**Validation**: `az account show`, Databricks workspace reachable (private endpoint if required).

### Phase 1 — Provision Infra (Terraform)

**Inputs**: SPNs (workload identity), VNets, subnets, storage account.
**Actions**: Deploy Databricks workspace, Storage (ADLS Gen2), Key Vault, Private Endpoints.
**Outputs**: MI principal ID, KV URI, storage endpoints.
**Validation**: Terraform plan/apply logs archived; Private Link approved; storage ACLs set.

### Phase 2 — Unity Catalog Bootstrap

**Inputs**: Workspace URL, MI, ADLS.
**Actions**: Run *§27 UC SQL* to create **storage credential**, **external location**, **catalogs** (`niloomid_dev/test/prod`), **schemas** (`raw/clean/gold/meta/ops`), **volumes**.
**Outputs**: UC objects created.
**Validation**: `SHOW CATALOGS; SHOW SCHEMAS IN niloomid_dev;` grants visible; lineage enabled.

### Phase 3 — Cluster Policies & Jobs

**Inputs**: Policy JSON (*§15*), job names.
**Actions**: Create cluster policies (Photon, single‑user, autotermination, tags). Register Databricks Workflows skeleton (*§30*).
**Outputs**: Policy IDs, workflow IDs.
**Validation**: Jobs enforce policy; attempts to change disallowed fields are blocked.

### Phase 4 — Secrets & Connections

**Inputs**: LLM keys, FAISS/Qdrant config, service creds.
**Actions**: Create Key Vault‑backed secret scopes; set Airflow connections (`databricks_default` via OAuth/OIDC).
**Outputs**: Secret scope names; tested tokenless auth.
**Validation**: Notebook can `dbutils.secrets.get` secrets; Airflow test connection OK.

### Phase 5 — Data Contracts & DQ Suites

**Inputs**: Source schemas, SLAs.
**Actions**: Author contract YAML (*§16*), map to GE suites (*§17.1*), enable CI contract diff.
**Outputs**: `/contracts/*.yml`, `/ge/suites/*.json|yml`.
**Validation**: CI fails on schema drift; GE suite runnable locally and in cluster.

### Phase 6 — Bronze Ingestion (Autoloader)

**Inputs**: Landing paths, schema registry path.
**Actions**: Configure Autoloader stream for JSON/CSV (*Bronze notebook* in §3.1); set checkpoint at `/mnt/lake/_chk/bronze/events`; enable `cloudFiles` options; batch jobs for historical backfill.
**Outputs**: `niloomid_{env}.raw.events_bronze` Delta table.
**Validation**: Stream active; ingest lag < 5m; schema inference saved; partitions visible.

### Phase 7 — Silver Cleanse & GE Gate

**Inputs**: Bronze table, GE suite.
**Actions**: Run *Silver notebook §3.2*; dedup, types, `event_dt` derived; apply **GE** gate; route failures to `ops.quarantine_events`.
**Outputs**: `niloomid_{env}.clean.events_silver`, quarantine table w/ run\_id.
**Validation**: GE pass ≥ 99%; quarantine empty; constraints enforced (*§29*).

### Phase 8 — Gold KPIs & Curated Text

**Inputs**: Silver tables.
**Actions**: Create KPIs and `gold.docs_text` (*§3.3*, *§29*).
**Outputs**: `gold.kpi_daily`, `gold.docs_text`.
**Validation**: Row counts match expectations; partitions present; downstream dashboards read OK.

### Phase 9 — Vectorization & Index

**Inputs**: `gold.docs_text`.
**Actions**: Clean/Chunk (*§3.4*), embed texts (*§3.4*), build FAISS index; (optional) Qdrant deploy via Docker/K8s.
**Outputs**: FAISS index artifact, or Qdrant collection.
**Validation**: kNN query returns relevant ids on smoke set; average cosine sim ≥ 0.6.

### Phase 10 — Agent & API

**Inputs**: Retriever, LLM key, prompt rules.
**Actions**: Implement LangGraph agent (*§3.5*), FastAPI service (*§3.6*, *§32*); add SLA guardrails; containerize.
**Outputs**: Docker image `niloomid/ai-api`; `/qa` endpoint responding.
**Validation**: p95 ≤ 2.5s on canary; citations present; no PII leakage in logs.

### Phase 11 — DLT & Airflow Orchestration

**Inputs**: DLT `pipeline.json`, notebooks, Airflow provider.
**Actions**: Import and run DLT in continuous mode (*§4*, *§18*); deploy Airflow DAG (*§31*) with task order `silver → gold → embed`.
**Outputs**: Running DLT pipeline; daily DAG runs green.
**Validation**: DLT expectations dropping bad rows; DAG success; SLA met.

### Phase 12 — CI/CD & Promotion Gates

**Inputs**: GitHub repo, workflows (*§6*, *§19*).
**Actions**: Enable Actions (lint, tests, GE, SAST, secret scan, DAB deploy); define environment protection rules; set promotion gate thresholds.
**Outputs**: Build badges green; dev → prod promotion via approvals.
**Validation**: All gates pass; bundle deploys; rollback tested.

### Phase 13 — Observability & SRE

**Inputs**: OTel exporter, log schema.
**Actions**: Emit structured logs/metrics/traces; dashboards for throughput/lag/GE pass/LLM cost; alerts for SLA/GE budget breaches.
**Outputs**: Grafana/DBSQL dashboards; alert rules.
**Validation**: Synthetic checks green; on‑call rotation active.

### Phase 14 — RAG Evaluation & Judgement

**Inputs**: Labeled eval set.
**Actions**: Run offline harness (*§26*), wire online judge; set circuit breakers.
**Outputs**: Daily `ops.rag_eval_metrics`; dashboards.
**Validation**: `hit_rate ≥ 0.85`, `faithfulness ≥ 0.75`; no regression after deploy.

### Phase 15 — Go‑Live & Runbooks

**Inputs**: All above.
**Actions**: Canary 10%; monitor for 24h; publish runbooks (*§8*, *§22*).
**Outputs**: Prod traffic 100%, rollback plan stored.
**Validation**: Cost & SLO steady; incident drills passed.
