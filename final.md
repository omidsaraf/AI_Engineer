...
9\. Join with static reference dims if needed (broadcast), still respecting watermark (Section **18.5**).
...
### Ordered one‑liners (quick run)

```bash
# 1) Provision UC & locations
terraform -chdir=infra/databricks apply -auto-approve

# 2) Create UC SQL objects & grants (if not TF-managed)
databricks sql -f sql/uc_setup.sql

# 3) Start Bronze Auto Loader and Silver transform
# (Jobs created via UI or API; or run notebooks ad hoc)
...
# 5) Build embeddings & indexes (Airflow task / make target)
python -m src.embed && python -m src.retriever

# 6) Run API locally
docker compose up api

# 7) Ship to AKS via Helm
helm upgrade --install niloomid-genai helm/niloomid-genai
```
