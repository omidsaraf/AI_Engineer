...
**Source→Domain mapping (Company-A):**
...
resource "azurerm_resource_group" "rg" {
  name     = "rg-Company-A-lakehouse"
  location = "australiaeast"
}

resource "azurerm_storage_account" "adls" {
  name                     = "Company-Alakehouseadls"
...
CAT, SCH, IDX = "ep_ai", "knowledge", "Company-A_docs_idx"
...
index = vsc.get_index("ep_ai.knowledge.Company-A_docs_idx")

QUESTION = dbutils.widgets.get("question") if dbutils.widgets else "What are Company-A data retention rules?"
...
## 23) Domain notes & key joins (Company-A)
...
## 24) Company-A Best Practices (Security, Governance, Engineering)
...
* Identity: Azure AD SSO and SCIM, groups per role: `Company-A-dpm-data-eng`, `Company-A-analyst-readers`, `Company-A-ml-serving`, `Company-A-ai-governance`.
...
SELECT *,
  CASE WHEN is_account_group_member('Company-A-analyst-readers') THEN email ELSE ep_curated.customer.mask_email(email) END AS email_safe
...
* SLAs: Airflow task-level SLAs; alert to `dpm_udp_devops@Company-A.com.au` and Teams webhook.
...
def on_failure(context):
    send_email(to=['dpm_udp_devops@Company-A.com.au'],
...
## 25) Company-A Domain Playbooks (Jarvis, AMDM, Pega, CMDM)
...
## 27) Next Steps (Company-A roll-out)
...
The following keys reflect Company-A Mandos conventions used in UDP sandpit → curated → SFDC flows. Replace only if your golden mappings differ.
...
* `azure_blob_Company-A` → ADLS connection string (if used).
...
ENVM=dev|test|prod
ALERT_EMAIL=dpm_udp_devops@Company-A.com.au
```
